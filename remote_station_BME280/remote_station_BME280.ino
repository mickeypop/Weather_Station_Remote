...
void setup() 
{
  ...
  initMDNS();     // setup mDNS,  RT_<MAC>.local, only last 4 char of MAC address ei; RT_A1B2.local
  ...
  {                                       // WiFi DOWN
   ...
   // try till reconnected or drop out at 200, usually < 30 once AP is up
    while (WiFi.status() != WL_CONNECTED && WLcount < 200 )
    {
      delay( 100 );
      Serial.printf(".");
         if (UpCount >= 50)  // just keep terminal from scrolling sideways forever
         {
            UpCount = 0;
            Serial.printf("\n");
         }
...
{ // both Rothfusz and Steadman's equations
  ...
  if (!isFahrenheit)
        temperature = convertC_to_F( temperature );
...
{  // http://192.168.1.120/set?temp=c&id=bedRm  for Celcius 
  ...
  if( changed )
  {
     ...
     {
        F_C_HOLD = F_C_PREF;
        Serial.printf( "Unit Config Saved: F_C_PREF = %s\n" , F_C_PREF.c_str());    
     }
  }
...
// builds the headless web return
